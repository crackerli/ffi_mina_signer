...
static void print_decode_x_im(unsigned char *src) {
...
void native_signDelegation() {
 ...
 // print_uint64_t(0x3fdf0efb0a5714);
 __android_log_print(ANDROID_LOG_DEBUG, "CK1", "field111 = %x", (kp.pub.x)[0]);
  ...
  bigint_to_string(buf, tmp);
  printf("field = %s\n", buf);
//  char *android_buf[128];
//  sprintf(android_buf, "field = %s\n", buf);
  __android_log_print(ANDROID_LOG_DEBUG, "CK1", "field = %s", buf);
  ...
  bigint_to_string(buf, tmp);
  printf("scalar = %s\n", buf);
  __android_log_print(ANDROID_LOG_DEBUG, "CK1", "scalar = %s", buf);
